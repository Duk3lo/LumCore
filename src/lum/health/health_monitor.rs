...
impl HealthMonitor {
        ...
        if let Ok(avg_tps) = avg_str.parse::<f64>() {
            if avg_tps < config.min_tps_threshold {
                ...
                if self.tps_strikes >= config.max_strikes {
                    println!(
                        "{RED}{BOLD}🚨 [HEALTH] FALLO CRÍTICO DE RENDIMIENTO. INICIANDO RESCATE DE EMERGENCIA...{RESET}"
                    );
                    self.execute_restart(server_cfg, server_runtime, core_tx);
                }
            } else if self.tps_strikes > 0 {
                println!(
                    "{GREEN}✅ [HEALTH] Estabilización confirmada ({:.2} TPS). Cancelando alertas de strike.{RESET}",
                    avg_tps
                );
                self.tps_strikes = 0;
            }
    ...
    pub fn print_health_status(&self, server_is_active: bool) {
        ...
        let status_icon = if self.enabled {
            "⚡ ACTIVE"
        } else {
            "💤 OFFLINE"
        };
        let server_icon = if server_is_active {
            format!("{GREEN}🟢 RUNNING{RESET}")
        } else {
            format!("{RED}🔴 DOWN{RESET}")
        };

        println!("\n{BLUE}╭──────────────────────────────────────────────────────────────╮{RESET}");
        println!(
            "{BLUE}│ {BOLD}{MAGENTA}🌌 CORENEXUS TELEMETRY {RESET}{BLUE}│ {ORANGE}🦀 Powered by Rust (Blazing Fast) {BLUE}│{RESET}"
        );
        println!("{BLUE}├──────────────────────────────────────────────────────────────┤{RESET}");
        ...
        println!(
            "{BLUE}│ {DIM}OS Target : {RESET}{}{RESET} ({})",
            OS.to_uppercase(),
            ARCH.to_uppercase()
        );
        ...
        // App Info
        println!(
            "{BLUE}│ {MAGENTA}🛡️  {} PROCESS {RESET}",
            app_name.to_uppercase()
        );
        println!("{BLUE}│ {DIM}PID       : {RESET}{}{RESET}", pid);
        println!(
            "{BLUE}│ {DIM}RAM Usage :{RESET} {GREEN}{}{RESET} {DIM}(Zero-Cost Abstractions){RESET}",
            format_memory(&app_mem)
        );
        ...
        if let Some(s_pid) = self.server_pid {
            ...
            println!(
                "{BLUE}│ {DIM}JVM CPU   : {RESET}{}",
                get_process_cpu_usage(s_pid)
            );
            println!(
                "{BLUE}│ {DIM}Threads   : {RESET}{}",
                get_process_thread_count(s_pid)
            );
            println!(
                "{BLUE}│ {DIM}Heap Info : {RESET}{}",
                get_jvm_heap_info(s_pid)
            );
        }
...
fn create_strike_bar(current: u32, max: u32) -> String {
    ...
    for i in 0..max {
        if i < current {
            bar.push_str(&format!("{RED}█{RESET}"));
        } else {
            bar.push_str(&format!("{GREEN}░{RESET}"));
        }
    }
...
fn get_process_thread_count(pid: u32) -> String {
    if cfg!(windows) {
            ...
            .output()
        {
            let text = String::from_utf8_lossy(&output.stdout);
            for line in text.lines() {
                if let Some(value) = line.trim().strip_prefix("ThreadCount=") {
...
fn get_jvm_heap_info(pid: u32) -> String {
    if let Ok(output) = Command::new("jcmd")
        .arg(pid.to_string())
        .arg("GC.heap_info")
        .output()
    {
