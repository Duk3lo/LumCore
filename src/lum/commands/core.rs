...
pub fn handle(input: &str, ctx: &mut CoreContext) -> bool {
    ...
    match sub.as_str() {
        "status" => {
            ...
            println!(
                "Servidor activo      : {}",
                if ctx.server_runtime.is_some() { "SÍ" } else { "NO" }
            );
            println!(
                "Ruta del JAR         : {}",
                if ctx.server_cfg.jar_path.is_empty() {
                    "No definida"
                } else {
                    &ctx.server_cfg.jar_path
                }
            );
        }
        ...
        "history" => {
            ...
            if ctx.command_history.is_empty() {
                ...
                for (i, cmd) in ctx.command_history.iter().enumerate() {
                    println!("{:03}: {}", i + 1, cmd);
                }
            }
        }
                ...
                "enable" | "disable" => {
                    ...
                    if modified {
                        ...
                        println!(
                            "[Updater] {} ha sido {}D.",
                            target.to_uppercase(),
                            if state { "ACTIVA" } else { "DESACTIVA" }
                        );
                        println!("[Updater] Usa 'core updater restart' para aplicar los cambios.");
                    }
                }
