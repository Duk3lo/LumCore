...
impl GitHubClient {
    ...
    fn get_latest_release(&self, repo_slug: &str, custom_token: &str) -> Result<GhRelease> {
        ...
        let token = if !custom_token.is_empty() {
            custom_token
        } else {
            &self.global_token
        };
    ...
    pub fn download_and_replace(
        &self,
        resource: &mut RepositoryResource,
        mod_key: &str,
    ) -> Result<bool> {
        ...
        if release.tag_name == resource.local_version_tag && file_exists {
            if resource.verify_file_integrity && !resource.last_verified_hash.is_empty() {
                ...
                println!(
                    "[GitHub] El mod {} ya está en su última versión ({}).",
                    resource.repo_slug, release.tag_name
                );
                return Ok(false);
            }
        }
        ...
        let target_asset = release
            ...
            .ok_or_else(|| {
                anyhow!(
                    "No se encontró archivo .jar o .zip en la versión {}",
                    release.tag_name
                )
            })?;

        println!(
            "[GitHub] Descargando actualización: {}...",
            target_asset.name
        );
        ...
        let mut req = self
            ...
            .header(USER_AGENT, "LumCore-Rust");
        let token = if !resource.custom_token.is_empty() {
            &resource.custom_token
        } else {
            &self.global_token
        };
        if !token.is_empty() {
        ...
        if resource.keep_backup {
            if let Some(old_name) = &resource.local_file_name {
                ...
                if old_path.exists() {
                    ...
                    fs::copy(
                        &old_path,
                        backup_dir.join(format!("{}.backup_{}", old_name, timestamp)),
                    )?;
                }
            }
        }
    ...
    pub fn restore_latest_backup(
        ...
        if !backup_dir.exists() {
            return Err(anyhow!("No hay backups para '{}'", mod_key));
        }
